...
def create_from_conversation(body: EvidenceFromConversationIn):
    try:
        res = create_evidences_from_latest_message(conversation_id=body.conversation_id, stage_id=body.stage_id, inline_threshold=body.inline_threshold)
...
@router.get("/mcp", response_model=McpEvidenceListOut, summary="MCP 증거 목록 조회")
