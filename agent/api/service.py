...
class LogCapture(io.StringIO):
    """stdout/stderr 캡처 (이벤트 루프 사용 안함 - MCP 클라이언트와 충돌 방지)"""
    ...
    def fileno(self):
        """MCP subprocess가 fileno()를 호출할 수 있으므로 지원"""
...
# class LogCapture(io.StringIO):
#     def __init__(self, trigger_id: str, original_stdout, loop):
#         super().__init__()
#         self.trigger_id = trigger_id
#         self.original_stdout = original_stdout
#         self.loop = loop
...
#                 send_log(self.trigger_id, text.rstrip()),
#                 self.loop
#             )
#         return len(text)
...
#         log_capture = LogCapture(trigger_id, original_stdout, loop)
#         sys.stdout = log_capture
#         sys.stderr = log_capture
