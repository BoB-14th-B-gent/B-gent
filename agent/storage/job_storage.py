...
def _get_client():
    ...
    if _client is None:
        try:
            ...
            debug_print(f"[X]  MongoDB 연결 실패 (작업은 계속됨): {e}")
            _client = None
            _db = None
...
def save_agent_state(
    ...
    try:
        ...
        debug_print(f"[X]  AGENT_STATES 저장 실패: {e}")
        return False
...
def add_mcp_tool(agent_id: str, mcp_name: str, tool_name: str, task_id: Optional[str] = None) -> bool:
    try:
        ...
        debug_print(f"[X]  MCP 도구 추가 실패: {e}")
        return False
...
def update_stage(agent_id: str, stage_id: int) -> bool:
    try:
        ...
        debug_print(f"[X]  스테이지 업데이트 실패: {e}")
        return False
...
def update_task_status(agent_id: str, task_id: str, task_status: str) -> bool:
    try:
        ...
        debug_print(f"[X]  Task 상태 업데이트 실패: {e}")
        return False
...
def is_first_execution_in_conversation(conversation_id: Optional[str]) -> bool:
    ...
    try:
        ...
        debug_print(f"[X]  첫 실행 여부 확인 실패: {e}")
        return True
